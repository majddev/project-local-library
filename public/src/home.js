...
function mostCommonGenres(books) {
  ...
  genreArray.forEach((value) => {
    ...
    const index = finalArray.findIndex(
      (indexNumber) => indexNumber.name === value
    );
...
function mostPopularBooks(books) {
  ...
  titleArray.forEach((title) => {
    ...
    const index = finalArray.findIndex(
      (indexNumber) => indexNumber.name === title
    );
...
function test(books, authors) {
  ...
  let finalArray = tempArray.reduce((accumulator, currentValue) => {
    let name = currentValue.name;
    let found = accumulator.find(author => author.name === name);
    if (found) found.count += currentValue.count;
    else accumulator.push(currentValue);
    return accumulator;
  }, []);
  return finalArray.sort((a, b) => (a.count < b.count ? 1 : -1)).slice(0, 5);
*/
let initialAuthorObj = {};
for (let i=0; i<books.length; i++) {
  let book = books[i];
  let authId = book.authorId;
  if (initialAuthorObj[authId]) {
    initialAuthorObj[authId] += book.borrows.length;
  } else {
    initialAuthorObj[authId] = book.borrows.length;
  }
}
//console.log(initialAuthorObj);
//console.log(Object.keys(initialAuthorObj))
  let tempArray = [];
  for (let auth in initialAuthorObj) {
    //console.log(auth)
    // Code goes here
    authors.forEach(author => {
      let secondAuthorObj = {};
      if (author.id === auth) {
        secondAuthorObj.name = `${author.name.first} + ${author.name.last}`;
        secondAuthorObj.count = initialAuthorObj[auth];
      } else {
        secondAuthorObj.count += initialAuthorObj[auth]
      }
      console.log(secondAuthorObj)
      tempArray.push(secondAuthorObj)
     });
...
console.log(test(books, authors));